...
- (void)layoutSubviews
{
    ...
    for (NSIndexPath *indexPath in self.indexPaths)
    {
        ...
        YLWaterFlowCell *cell = self.screenCellsDict[indexPath];
    ...
    if (self.contentOffset.y + self.bounds.size.height > self.contentSize.height) {
        ...
        // 加载数据完成后，通知视图刷新显示
        if (self.delegate && [self.delegate respondsToSelector:@selector(waterFlowViewRefreshData:)])
        {
...
- (void)resetView
{
    ...
    YLLog(@"%f----%f",colW,self.bounds.size.width);
    // 2) 使用一个数组，记录每列的当前Y值
